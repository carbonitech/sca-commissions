...
class Processor:
    ...
    def add_branch_id(self, do_auto_match: bool = True) -> "Processor":
        ...
        if not unmatched_id_strings.empty and do_auto_match:
            auto_matched = self.attempt_auto_matching(
                unmatched_rows=unmatched_id_strings
            )
            if not auto_matched.empty:
                auto_matched_index = auto_matched.index
                # fill the data with auto_matched values
                operating_data.loc[auto_matched_index, combined_new_cols] = (
                    auto_matched.loc[auto_matched_index, combined_new_cols]
                )
        operating_data = self._filter_out_any_rows_unmapped(
            operating_data, error_type=ErrorType(4)
        )
        self.staged_data = operating_data
        return self

    def _filter_out_any_rows_unmapped(
        self, data: pd.DataFrame, error_type: ErrorType
    ) -> pd.DataFrame:
        if data.empty:
            return data
        mask = data.loc[
            :, ~data.columns.isin(["submission_id", "inv_amt", "comm_amt"])
        ].all("columns")
        data_remaining = data[mask]
        data_removed: pd.DataFrame = data.loc[
            ~mask, ~data.columns.isin(["customer_branch_id"])
        ]
    ...
    def preprocess(self) -> "Processor":
        report_name = get.report_name_by_id(
            db=self.session, report_id=self.submission.report_id
        )
        sub_id = self.submission_id
        file = self.submission.file
        preprocessor: AbstractPreProcessor = self.preprocessor(
            report_name, sub_id, file
        )
        optional_params = {
...
class NewReportStrategy(Processor):
    def __init__(
        ...
        self.column_names = get.report_column_names(
            session, report_id=self.submission.report_id
        )
    ...
    def set_submission_status(self, status: str) -> "Processor":
        ...
        if status == "COMPLETE":
            if self.session.execute(
                "SELECT * FROM errors WHERE submission_id = :sub_id LIMIT 1;",
                {"sub_id": self.submission_id},
            ).fetchone():
                return self
            else:
        ...
        try:
            (
                ...
                .set_submission_status(
                    "COMPLETE"
                )  # If there aren't any errors found in the db
            )
...
class ErrorReintegrationStrategy(Processor):
    ...
    def __init__(
        ...
        self.error_table: pd.DataFrame = pd.concat(
            [error_table, pd.json_normalize(error_table.pop("row_data"), max_level=1)],
            axis=1,
        )
    ...
    def insert_report_id(self) -> "Processor":
        new_col = (
            self.staged_data.loc[:, ["submission_id"]]
            .merge(self.report_id_by_submission, how="left", on="submission_id")
            .loc[:, ["report_id"]]
        )
