...
class PreProcessor(AbstractPreProcessor):
    ...
    def _standard_report_preprocessing(self, data: pd.Series, **kwargs) -> PreProcessedData:
        ...
        # reduce to a slice, using the series value that contains the "Sales Data" header, but leave out the header and totals rows
        data: pd.Series = data[data.index[data.str.match(r'^\s*Sales Data')].tolist()[0]:].iloc[2:-1]
        df = (
        ...
        replace_values: pd.Series = (
            df.iloc[:,1:]
                .apply(lambda row: [row.index[row>0].max(), row.loc[row.index[row > 0].max()]] if row.iloc[-1] == 0 else 0, axis=1)
                .replace(0, np.nan)
                .dropna()
        )
    ...
    def preprocess(self, **kwargs) -> PreProcessedData:
        ...
        if preprocess_method:
            return preprocess_method(self.file.to_df(pdf="text", combine_sheets=True), **kwargs)
        else:
            return