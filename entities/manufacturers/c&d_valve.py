...
class PreProcessor(AbstractPreProcessor):
    ...
    def _standard_report_preprocessing(
        ...
        try:
            ...
            return self.parse_styled_excel(data, sales_report, **kwargs)
        except:
            sales_report: pd.DataFrame = pd.read_csv(sales_report, header=None)
            return self.parse_plain_csv_sales_report(data, sales_report, **kwargs)

    def parse_styled_excel(
        self, data: pd.DataFrame, sales_report: pd.DataFrame, **kwargs
    ) -> PreProcessedData:
        """
        processes the C&D standard report
        ...
        sales_report.rename(
            columns=lambda col: str(col).lower().replace(" ", ""), inplace=True
        )
        ...
        # in the sales report, inoice numbers are in their own column
        # and share the same index as customer info and sales figure
    ...
    def parse_plain_csv_sales_report(
        self, data: pd.DataFrame, sales_report: pd.DataFrame, **kwargs
    ) -> PreProcessedData:
