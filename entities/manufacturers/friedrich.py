...
class PreProcessor(AbstractPreProcessor):
    ...
    def _standard_report_preprocessing(
        ...
        result = result[["id_string", "inv_amt", "comm_amt"]].astype(
            self.EXPECTED_TYPES
        )
    ...
    def _johnstone_report_preprocessing(
        ...
        data = self.check_headers_and_fix(
            [store_number_col, city_name_col, state_name_col, inv_col, comm_col], data
        )
        ...
        result.loc[:, comm_col] *= 100
        result["id_string"] = (
            result[[store_number_col, "customer", city_name_col, state_name_col]]
            .fillna("")
            .apply("_".join, axis=1)
        )
    ...
    def _ferguson_report_preprocessing(
        self, data: pd.DataFrame, **kwargs
    ) -> PreProcessedData:
        ...
        result.loc[:, comm] *= 100
        result["id_string"] = (
            result[["customer", ship_to, state]].fillna("").apply("_".join, axis=1)
        )
