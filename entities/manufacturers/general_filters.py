...
class PreProcessor(AbstractPreProcessor):
    ...
    def _standard_report_preprocessing(
        ...
        data = data[1:]
        data = data.loc[
            ~data.isin(data[:4]) & ~data.str.contains(page_num_re)
        ].reset_index(drop=True)
        customer_boundaries: list[int] = [
            index + offset
            for index in data.loc[
                data.str.contains(customer_boundary_re)
            ].index.tolist()
        ]
        compiled_data = {"customer": [], "location": [], "inv_amt": [], "comm_amt": []}
