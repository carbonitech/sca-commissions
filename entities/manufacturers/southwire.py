...
class PreProcessor(AbstractPreProcessor):
    ...
    def _baker_report_preprocessing(
        ...
        branch_proportions: pd.DataFrame = kwargs.get(
            "customer_proportions_by_state", pd.DataFrame()
        )
        ...
        if not branch_proportions.empty:
            branch_proportions["inv_amt"] = (
                branch_proportions["total_share"] * total_inv
            )
            branch_proportions["comm_amt"] = (
                branch_proportions["total_share"] * total_comm
            )
            branch_proportions["id_string"] = branch_proportions[
                ["customer", "state"]
            ].apply("_".join, axis=1)
            ...
            result = pd.DataFrame(
                [[specified_customer, total_inv, total_comm]],
                columns=["id_string", "inv_amt", "comm_amt"],
            )
        return PreProcessedData(result)
    ...
    def _winsupply_report_preprocessing(
        ...
        if not comm_col:
            ...
            data.loc[:, "comm_amt"] = data.loc[:, comm_col]
            comm_col: str = "comm_amt"
    ...
    def _rebate_detail_report_preprocessing(
        self, data: pd.DataFrame, **kwargs
    ) -> PreProcessedData:
    ...
    def _johnstone_report_preprocessing(
        ...
        comm_rate: float = float(kwargs.get("standard_commission_rate", 0))
    ...
    def preprocess(self, **kwargs) -> PreProcessedData:
        ...
        if preprocess_method:
            ...
            return preprocess_method(
                self.file.to_df(skip=skip_param, treat_headers=True), **kwargs
            )
        else:
            return
