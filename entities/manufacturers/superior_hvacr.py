...
class PreProcessor(AbstractPreProcessor):
    ...
    def _standard_report_preprocessing(
        ...
        result = data.merge(
            sales_data,
            left_on=customer_number_col_comm_file,
            right_on=customer_number_col_sales_file,
        )
        result = result.loc[
            :,
            [
                customer_name_col_comm_file,
                city_name_col_sales_file,
                inv_col_comm_file,
                comm_col_comm_file,
            ],
        ]
    ...
    def _united_refrigeration_report_preprocessing(
        ...
        data = pd.concat(
            [data.loc[:, id_cols], data.iloc[:, [inv_amt, comm_amt]]], axis=1
        )
