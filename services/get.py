...
def report_calendar(db: Session, user: User) -> ReportCalendar:
    ...
    submitted_list = submitted_list[['name','report_label','reporting_month','reporting_year','total_commission_amount']]
    ...
    filled_list = report_full_list.merge(submitted_list, how='left', on=['name','report_label','reporting_month','reporting_year'])
    # make a date column
    filled_list['date'] = pd.to_datetime(
        filled_list['reporting_year'].astype(str)+"-"+filled_list['reporting_month'].astype(str)+"-01"
        ).dt.date.astype(str)
    filled_list = filled_list.rename(columns={'name': 'manufacturer'})
    response = [ReportRecord(**record) for record in json.loads(filled_list.to_json(orient='records'))]
